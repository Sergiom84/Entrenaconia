/**
 * Funcional Manual Card - Arquitectura Modular Profesional v1.0
 * Componente principal para evaluación y selección de metodología funcional
 * Basado en el patrón arquitectural establecido por Calistenia
 *
...
const CARD_CONFIG = {
  API_ENDPOINTS: {
    EVALUATE_PROFILE: '/api/funcional-specialist/evaluate-profile'
  },
  PROGRESSION: {
    REQUIRED_COMPLETION_RATE: 80,
    BASE_WORKOUT_TIME: 50
  },
  VERSION: {
    COMPONENT: '1.0',
    API: '1.0'
...
import { FUNCIONAL_LEVELS, getLevelConfig, getLevelRecommendations } from './FuncionalLevels.js';
import { getMuscleGroupInfo, getRecommendedGroupsByLevel, generateBalancedSplit } from './FuncionalMuscleGroups.js';
...
export default function FuncionalManualCard({ onGenerate, isLoading, error }) {
  ...
  const evaluateUserProfile = async () => {
    ...
    try {
      console.log(`🤖 Iniciando evaluación automática Funcional v${CARD_CONFIG.VERSION.COMPONENT}...`);
  ...
  const generateWithAI = async () => {
    ...
    try {
      console.log('🚀 Generando plan funcional con IA especializada...');
      ...
      const funcionalData = {
        methodology: 'Funcional Specialist',
  ...
  const EvaluationHeader = () => (
    <div className="text-center mb-8">
      <div className="flex justify-center items-center gap-3 mb-4">
        ...
        <h2 className="text-3xl font-bold text-white">Evaluación IA Funcional</h2>
      ...
      <p className="text-gray-400 max-w-2xl mx-auto">
        Nuestro sistema IA especializado evalúa tu perfil para recomendarte el nivel óptimo de entrenamiento funcional
      </p>
    </div>
  );
  ...
  const LoadingEvaluationState = () => (
    <div className="text-center py-8">
      ...
      <p className="text-gray-400 mb-3">
        La IA está evaluando tu experiencia, objetivos y capacidades funcionales actuales
      </p>
  ...
  const ErrorEvaluationState = () => (
    <div className="text-center py-8">
      ...
      <button
        ...
        className={`px-4 py-2 bg-${CARD_CONFIG.THEME.PRIMARY} text-black rounded-lg hover:bg-emerald-300 transition-colors`}
      >
  ...
  const ManualHeader = () => (
    <div className="text-center mb-8">
      ...
      <p className="text-gray-400 max-w-2xl mx-auto">
        Elige tu nivel basándote en tu experiencia actual en entrenamiento funcional
      </p>
    </div>
  );
  ...
  const LevelSelectionGrid = () => (
    <div className="mb-8">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {Object.entries(FUNCIONAL_LEVELS).map(([key, level]) => (
          <div
            key={key}
    ...
    return (
      <div className={`mb-8 p-6 bg-${CARD_CONFIG.THEME.BACKGROUND} border border-${CARD_CONFIG.THEME.BORDER} rounded-xl`}>
        <h3 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
          ...
          Hitos para nivel {FUNCIONAL_LEVELS[state.selectedLevel].name}
        </h3>
        <div className="grid md:grid-cols-2 gap-3 mb-4">
          {FUNCIONAL_LEVELS[state.selectedLevel].hitos.map((hito, index) => (
    ...
    return (
      <div className="mb-8">
        ...
        <textarea
          ...
          placeholder="Ej: Mejorar movilidad de hombro, desarrollar explosividad en piernas, aumentar fuerza relativa, mejorar estabilidad de core..."
    ...
    <div className="p-6 bg-black text-white min-h-[80vh]">
      {isLoading && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60]">
          <div className="bg-black/90 border border-emerald-400/30 rounded-lg p-6 text-center shadow-xl">
            <svg className="w-10 h-10 text-emerald-400 animate-spin mx-auto mb-3" fill="none" viewBox="0 0 24 24">
              <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
              <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path>
            </svg>
            <p className="text-white font-semibold">La IA está generando el entrenamiento funcional</p>
          </div>
        </div>
      )}
