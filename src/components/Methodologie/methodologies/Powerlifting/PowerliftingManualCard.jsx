/**
 * Powerlifting Manual Card - Arquitectura Modular Profesional v1.0
 * Componente principal para evaluación y selección de metodología Powerlifting
 * Basado en el patrón de HipertrofiaManualCard.jsx
 *
 * @author Claude Code - Arquitectura Modular Avanzada
 * @version 1.0.0 - Powerlifting Implementation
 */
...
const CARD_CONFIG = {
  API_ENDPOINTS: {
    EVALUATE_PROFILE: '/api/powerlifting-specialist/evaluate-profile'
  },
  PROGRESSION: {
    MIN_REST_DAYS: 2,
    MAX_SETS_PER_EXERCISE: 8,
    BASE_WORKOUT_TIME: 90
  },
  VERSION: {
    COMPONENT: '1.0',
    API: '1.0'
...
import { POWERLIFTING_LEVELS, getLevelConfig, getLevelRecommendations } from './PowerliftingLevels.js';
import { getMuscleGroupInfo, getRecommendedGroupsByLevel, generateBalancedSplit } from './PowerliftingMuscleGroups.js';
...
export default function PowerliftingManualCard({ onGenerate, isLoading, error }) {
  ...
  /**
   * Evaluación automática del perfil con IA especializada - v1.0 Powerlifting
   */
  const evaluateUserProfile = async () => {
    dispatch({ type: 'SET_EVALUATION_LOADING', payload: true });

    try {
      console.log(`🏋️ Iniciando evaluación Powerlifting v${CARD_CONFIG.VERSION.COMPONENT}...`);
      ...
      console.error('❌ Error en evaluación IA Powerlifting:', error);
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      const powerliftingData = {
        methodology: 'Powerlifting Specialist',
      ...
      onGenerate(powerliftingData);

    } catch (error) {
      console.error('❌ Error generando con IA Powerlifting:', error);
      dispatch({ type: 'SET_EVALUATION_ERROR', payload: error.message });
    }
  };
  ...
  const generateManually = () => {
    ...
    const powerliftingData = {
      methodology: 'Powerlifting Manual',
  ...
  const EvaluationHeader = () => (
    <div className="text-center mb-8">
      <div className="flex justify-center items-center gap-3 mb-4">
        <div className={`p-3 bg-${CARD_CONFIG.THEME.POWERLIFTING}/10 rounded-full`}>
          <Award className={`w-8 h-8 text-${CARD_CONFIG.THEME.POWERLIFTING}`} />
        </div>
        <h2 className="text-3xl font-bold text-white">Evaluación IA Powerlifting</h2>
      ...
      <p className="text-gray-400 max-w-2xl mx-auto">
        Nuestro sistema IA especializado evalúa tu perfil para determinar tu nivel óptimo en Powerlifting
      </p>
      <div className="mt-4 p-3 bg-red-600/10 border border-red-600/30 rounded-lg max-w-2xl mx-auto">
        <p className="text-red-300 text-sm">
          <strong>🏋️ Powerlifting:</strong> Fuerza máxima en Sentadilla, Press de Banca y Peso Muerto. Periodización inteligente para récords personales.
        </p>
      </div>
    </div>
  );
  ...
  const LoadingEvaluationState = () => (
    <div className="text-center py-8">
      <div className="relative">
        <Loader className={`w-12 h-12 animate-spin text-${CARD_CONFIG.THEME.POWERLIFTING} mx-auto mb-4`} />
        <div className={`absolute -top-1 -right-1 w-4 h-4 bg-${CARD_CONFIG.THEME.SUCCESS} rounded-full animate-pulse`}></div>
      </div>
      <h3 className="text-xl font-semibold text-white mb-2">Analizando tu perfil Powerlifting...</h3>
      <p className="text-gray-400 mb-3">
        La IA está evaluando tu experiencia con levantamientos pesados, técnica y capacidad de recuperación
      </p>
      <div className={`inline-flex items-center gap-2 px-3 py-1 bg-${CARD_CONFIG.THEME.POWERLIFTING}/10 text-${CARD_CONFIG.THEME.POWERLIFTING} rounded-full text-sm border border-${CARD_CONFIG.THEME.POWERLIFTING}/30`}>
        <Award className="w-4 h-4" />
        Powerlifting v{CARD_CONFIG.VERSION.API} - Fuerza Máxima
      </div>
    </div>
  );
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            <h3 className="text-xl font-semibold text-white mb-2">
              Nivel Recomendado: <span className={`text-${CARD_CONFIG.THEME.POWERLIFTING}`}>
            ...
            <div className="flex items-center gap-2 mb-3">
              <div className="flex-1 bg-gray-700 rounded-full h-2">
                <div
                  className={`bg-${CARD_CONFIG.THEME.POWERLIFTING} h-2 rounded-full`}
            ...
            <div className="grid md:grid-cols-2 gap-2">
              {state.aiEvaluation.key_indicators.map((indicator, index) => (
                <div key={index} className="flex items-center gap-2 text-sm text-gray-300">
                  <Award className={`w-4 h-4 text-${CARD_CONFIG.THEME.POWERLIFTING} flex-shrink-0`} />
                  {indicator}
                </div>
              ))}
            </div>
        ...
        {state.aiEvaluation.suggested_focus_areas?.length > 0 && (
          <div className="mb-6">
            <h4 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
              ...
              Levantamientos Prioritarios
            </h4>
            <div className="flex flex-wrap gap-2">
              {state.aiEvaluation.suggested_focus_areas.map((area, index) => (
                <span
                  key={index}
                  className={`px-3 py-1 bg-${CARD_CONFIG.THEME.POWERLIFTING}/10 text-${CARD_CONFIG.THEME.POWERLIFTING} border border-${CARD_CONFIG.THEME.POWERLIFTING}/30 rounded-full text-sm`}
                >
                  {area}
                </span>
              ))}
          ...
          <button
            ...
            className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all ${
              isLoading
                ...
                : `bg-${CARD_CONFIG.THEME.POWERLIFTING} text-white hover:bg-red-700 transform hover:scale-[1.02]`
            }`}
          >
            <div className="flex items-center justify-center gap-2">
              <Award className="w-5 h-5" />
              {isLoading ? 'Generando...' : 'Generar Plan Powerlifting con IA'}
            </div>
          </button>
  ...
  const ManualHeader = () => (
    <div className="text-center mb-8">
      <div className="flex justify-center items-center gap-3 mb-4">
        ...
        <h2 className="text-3xl font-bold text-white">Selección Manual Powerlifting</h2>
      </div>
      <p className="text-gray-400 max-w-2xl mx-auto">
        Elige tu nivel basándote en tu experiencia con levantamientos pesados y competencias
      </p>
    </div>
  );
  ...
  const LevelSelectionGrid = () => (
    <div className="mb-8">
      <h3 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
        <Target className={`w-5 h-5 text-${CARD_CONFIG.THEME.PRIMARY}`} />
        Selecciona tu nivel Powerlifting
      </h3>
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
        {Object.entries(POWERLIFTING_LEVELS).map(([key, level]) => (
          <div
            key={key}
            className={`p-5 rounded-xl border-2 cursor-pointer transition-all duration-300 hover:shadow-lg ${
              state.selectedLevel === key
                ? `bg-${CARD_CONFIG.THEME.POWERLIFTING}/10 border-${CARD_CONFIG.THEME.POWERLIFTING}/60 shadow-lg scale-105`
                : `bg-${CARD_CONFIG.THEME.BACKGROUND} border-${CARD_CONFIG.THEME.BORDER} hover:border-${CARD_CONFIG.THEME.POWERLIFTING}/40`
            }`}
          ...
          >
            ...
            <div className="flex items-center gap-2 text-sm text-red-300">
              <Award className={`w-4 h-4 text-${CARD_CONFIG.THEME.POWERLIFTING}`} />
              {level.technical?.intensityRange}
            </div>
          </div>
        ))}
      </div>
  ...
  const SelectedLevelHitos = () => {
    ...
    return (
      <div className={`mb-8 p-6 bg-${CARD_CONFIG.THEME.BACKGROUND} border border-${CARD_CONFIG.THEME.BORDER} rounded-xl`}>
        <h3 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
          <Zap className={`w-5 h-5 text-${CARD_CONFIG.THEME.POWERLIFTING}`} />
          Hitos para nivel {POWERLIFTING_LEVELS[state.selectedLevel].name}
        </h3>
        <div className="grid md:grid-cols-2 gap-3 mb-4">
          {POWERLIFTING_LEVELS[state.selectedLevel].hitos.map((hito, index) => (
            <div key={index} className="flex items-start gap-3 p-3 bg-black/60 rounded-lg">
              <Award className={`w-5 h-5 text-${CARD_CONFIG.THEME.POWERLIFTING} mt-0.5 flex-shrink-0`} />
    ...
    return (
      <div className="mb-8">
        ...
        <textarea
          ...
          placeholder="Ej: Mejorar sentadilla, corregir técnica en press de banca, preparar competencia..."
          className={`w-full p-4 bg-${CARD_CONFIG.THEME.BACKGROUND} border border-${CARD_CONFIG.THEME.PRIMARY}/30 text-white placeholder-gray-500 rounded-lg resize-none h-24 focus:ring-2 focus:ring-${CARD_CONFIG.THEME.POWERLIFTING} focus:border-transparent`}
        />
      </div>
    );
  };
        ...
        <button
          ...
          className={`flex-1 px-8 py-3 rounded-xl text-white font-semibold transition-all ${
            isLoading
              ...
              : `bg-${CARD_CONFIG.THEME.POWERLIFTING} hover:bg-red-700 transform hover:scale-[1.02]`
          }`}
        >
          {isLoading ? (
            <div className="flex items-center justify-center gap-2">
              <Loader className="w-5 h-5 animate-spin" />
              Generando plan Powerlifting...
            </div>
          ) : (
            <div className="flex items-center justify-center gap-2">
              <Award className="w-5 h-5" />
              Generar Plan Powerlifting Manual
            </div>
          )}
        </button>
      </div>
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60]">
          <div className="bg-black/90 border border-red-600/30 rounded-lg p-6 text-center shadow-xl">
            ...
            <p className="text-white font-semibold">La IA está generando tu plan Powerlifting</p>
            <p className="text-gray-400 text-sm mt-2">Fuerza máxima optimizada...</p>
          </div>
        </div>
      )}
