/**
 * Hipertrofia Manual Card - Arquitectura Modular Profesional v1.0
 * Componente principal para evaluación y selección de metodología Hipertrofia
 ...
 * @version 1.0.0 - Hipertrofia Implementation
 */
...
import { HIPERTROFIA_LEVELS, getLevelConfig, getLevelRecommendations } from './HipertrofiaLevels.js';
import { getMuscleGroupInfo, getRecommendedGroupsByLevel, generateBalancedSplit } from './HipertrofiaMuscleGroups.js';
...
export default function HipertrofiaManualCard({ onGenerate, isLoading, error }) {
  ...
  /**
   * Evaluación automática del perfil con IA especializada - v1.0 Hipertrofia
   */
  const evaluateUserProfile = async () => {
    dispatch({ type: 'SET_EVALUATION_LOADING', payload: true });

    try {
      console.log(`💪 Iniciando evaluación Hipertrofia v${CARD_CONFIG.VERSION.COMPONENT}...`);
      ...
      console.log('✅ Evaluación Hipertrofia completada:', {
      ...
      console.error('❌ Error en evaluación IA Hipertrofia:', error);
  ...
  const generateWithAI = async () => {
    ...
    try {
      console.log('🚀 Generando plan Hipertrofia con IA especializada...');

      const fullProfile = {
        id: userData?.id || user?.id || currentUser?.id
      };

      const hipertrofiaData = {
      ...
      console.error('❌ Error generando con IA Hipertrofia:', error);
      dispatch({ type: 'SET_EVALUATION_ERROR', payload: error.message });
    }
  };
  ...
  const generateManually = () => {
    ...
    const hipertrofiaData = {
      methodology: 'Hipertrofia Manual',
  ...
  const EvaluationHeader = () => (
    <div className="text-center mb-8">
      <div className="flex justify-center items-center gap-3 mb-4">
        <div className={`p-3 bg-${CARD_CONFIG.THEME.HIPERTROFIA}/10 rounded-full`}>
          <Dumbbell className={`w-8 h-8 text-${CARD_CONFIG.THEME.HIPERTROFIA}`} />
        </div>
        <h2 className="text-3xl font-bold text-white">Evaluación IA Hipertrofia</h2>
      ...
      <p className="text-gray-400 max-w-2xl mx-auto">
        Nuestro sistema IA especializado evalúa tu perfil para determinar tu nivel óptimo en Hipertrofia
      </p>
      <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg max-w-2xl mx-auto">
        <p className="text-blue-300 text-sm">
          <strong>💪 Hipertrofia:</strong> Volumen progresivo, periodización inteligente y técnicas de intensidad para máximo crecimiento muscular.
        </p>
      </div>
    </div>
  );
  ...
  const LoadingEvaluationState = () => (
    <div className="text-center py-8">
      <div className="relative">
        <Loader className={`w-12 h-12 animate-spin text-${CARD_CONFIG.THEME.HIPERTROFIA} mx-auto mb-4`} />
        <div className={`absolute -top-1 -right-1 w-4 h-4 bg-${CARD_CONFIG.THEME.SUCCESS} rounded-full animate-pulse`}></div>
      </div>
      <h3 className="text-xl font-semibold text-white mb-2">Analizando tu perfil Hipertrofia...</h3>
      <p className="text-gray-400 mb-3">
        La IA está evaluando tu experiencia, tolerancia al volumen y capacidad de recuperación
      </p>
      <div className={`inline-flex items-center gap-2 px-3 py-1 bg-${CARD_CONFIG.THEME.HIPERTROFIA}/10 text-${CARD_CONFIG.THEME.HIPERTROFIA} rounded-full text-sm border border-${CARD_CONFIG.THEME.HIPERTROFIA}/30`}>
        <Dumbbell className="w-4 h-4" />
        Hipertrofia v{CARD_CONFIG.VERSION.API} - Volumen Progresivo
      </div>
    </div>
  );
  ...
  const EvaluationResultSection = () => {
    ...
    return (
      <div>
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            <h3 className="text-xl font-semibold text-white mb-2">
              Nivel Recomendado: <span className={`text-${CARD_CONFIG.THEME.HIPERTROFIA}`}>
            ...
            <div className="flex items-center gap-2 mb-3">
              <div className="flex-1 bg-gray-700 rounded-full h-2">
                <div
                  className={`bg-${CARD_CONFIG.THEME.HIPERTROFIA} h-2 rounded-full`}
            ...
            <div className="grid md:grid-cols-2 gap-2">
              {state.aiEvaluation.key_indicators.map((indicator, index) => (
                <div key={index} className="flex items-center gap-2 text-sm text-gray-300">
                  <Dumbbell className={`w-4 h-4 text-${CARD_CONFIG.THEME.HIPERTROFIA} flex-shrink-0`} />
                  {indicator}
                </div>
              ))}
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {state.aiEvaluation.suggested_focus_areas.map((area, index) => (
                <span
                  key={index}
                  className={`px-3 py-1 bg-${CARD_CONFIG.THEME.HIPERTROFIA}/10 text-${CARD_CONFIG.THEME.HIPERTROFIA} border border-${CARD_CONFIG.THEME.HIPERTROFIA}/30 rounded-full text-sm`}
                >
                  {area}
                </span>
              ))}
        ...
        {/* Volumen tolerado */}
        {state.aiEvaluation.volume_tolerance && (
          <div className="mb-6">
            <h4 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
            ...
            <div className={`bg-${CARD_CONFIG.THEME.WARNING}/10 border border-${CARD_CONFIG.THEME.WARNING}/30 rounded-lg p-3`}>
              <p className="text-orange-300 text-sm">
                {state.aiEvaluation.volume_tolerance === 'alta' ? '✅ Alta tolerancia al volumen - puedes manejar 20-25 series/semana/músculo' :
                 state.aiEvaluation.volume_tolerance === 'media' ? '⚡ Tolerancia media - comienza con 15-20 series/semana/músculo' :
                 '⚠️ Baja tolerancia - limita a 10-15 series/semana/músculo inicialmente'}
              </p>
            </div>
          </div>
        )}
          ...
          <button
            ...
            className={`flex-1 px-6 py-3 rounded-xl font-semibold transition-all ${
              isLoading
                ...
                : `bg-${CARD_CONFIG.THEME.HIPERTROFIA} text-white hover:bg-blue-600 transform hover:scale-[1.02]`
            }`}
          >
            <div className="flex items-center justify-center gap-2">
              <Dumbbell className="w-5 h-5" />
              {isLoading ? 'Generando...' : 'Generar Plan Hipertrofia con IA'}
            </div>
          </button>
  ...
  const ManualHeader = () => (
    <div className="text-center mb-8">
      <div className="flex justify-center items-center gap-3 mb-4">
        ...
        <h2 className="text-3xl font-bold text-white">Selección Manual Hipertrofia</h2>
      </div>
      <p className="text-gray-400 max-w-2xl mx-auto">
        Elige tu nivel basándote en tu experiencia con entrenamiento de volumen y periodización
      </p>
    </div>
  );
  ...
  const LevelSelectionGrid = () => (
    <div className="mb-8">
      <h3 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
        <Target className={`w-5 h-5 text-${CARD_CONFIG.THEME.PRIMARY}`} />
        Selecciona tu nivel Hipertrofia
      </h3>
      <div className="grid md:grid-cols-3 gap-4">
        {Object.entries(HIPERTROFIA_LEVELS).map(([key, level]) => (
          <div
            key={key}
            className={`p-5 rounded-xl border-2 cursor-pointer transition-all duration-300 hover:shadow-lg ${
              state.selectedLevel === key
                ? `bg-${CARD_CONFIG.THEME.HIPERTROFIA}/10 border-${CARD_CONFIG.THEME.HIPERTROFIA}/60 shadow-lg scale-105`
                : `bg-${CARD_CONFIG.THEME.BACKGROUND} border-${CARD_CONFIG.THEME.BORDER} hover:border-${CARD_CONFIG.THEME.HIPERTROFIA}/40`
            }`}
          ...
          >
            ...
            <div className="flex items-center gap-2 text-sm text-blue-300">
              <Dumbbell className={`w-4 h-4 text-${CARD_CONFIG.THEME.HIPERTROFIA}`} />
              {level.technical?.setsPerExercise}
            </div>
          </div>
        ))}
      </div>
  ...
  const SelectedLevelHitos = () => {
    ...
    return (
      <div className={`mb-8 p-6 bg-${CARD_CONFIG.THEME.BACKGROUND} border border-${CARD_CONFIG.THEME.BORDER} rounded-xl`}>
        <h3 className="text-xl font-semibold mb-4 text-white flex items-center gap-2">
          <Zap className={`w-5 h-5 text-${CARD_CONFIG.THEME.HIPERTROFIA}`} />
          Hitos para nivel {HIPERTROFIA_LEVELS[state.selectedLevel].name}
        </h3>
        <div className="grid md:grid-cols-2 gap-3 mb-4">
          {HIPERTROFIA_LEVELS[state.selectedLevel].hitos.map((hito, index) => (
            <div key={index} className="flex items-start gap-3 p-3 bg-black/60 rounded-lg">
              <Dumbbell className={`w-5 h-5 text-${CARD_CONFIG.THEME.HIPERTROFIA} mt-0.5 flex-shrink-0`} />
    ...
    return (
      <div className="mb-8">
        ...
        <textarea
          ...
          placeholder="Ej: Aumentar pecho y espalda, mejorar brazos, equilibrar desarrollo muscular..."
          className={`w-full p-4 bg-${CARD_CONFIG.THEME.BACKGROUND} border border-${CARD_CONFIG.THEME.PRIMARY}/30 text-white placeholder-gray-500 rounded-lg resize-none h-24 focus:ring-2 focus:ring-${CARD_CONFIG.THEME.HIPERTROFIA} focus:border-transparent`}
        />
      </div>
    );
  };
        ...
        <button
          ...
          className={`flex-1 px-8 py-3 rounded-xl text-white font-semibold transition-all ${
            isLoading
              ...
              : `bg-${CARD_CONFIG.THEME.HIPERTROFIA} hover:bg-blue-600 transform hover:scale-[1.02]`
          }`}
        >
          {isLoading ? (
            <div className="flex items-center justify-center gap-2">
              <Loader className="w-5 h-5 animate-spin" />
              Generando plan Hipertrofia...
            </div>
          ) : (
            <div className="flex items-center justify-center gap-2">
              <Dumbbell className="w-5 h-5" />
              Generar Plan Hipertrofia Manual
            </div>
          )}
        </button>
      </div>
    ...
    <div className="p-6 bg-black text-white min-h-[80vh]">
      {isLoading && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60]">
          <div className="bg-black/90 border border-blue-500/30 rounded-lg p-6 text-center shadow-xl">
            ...
            <p className="text-white font-semibold">La IA está generando tu plan Hipertrofia</p>
            <p className="text-gray-400 text-sm mt-2">Volumen progresivo optimizado...</p>
          </div>
        </div>
      )}
